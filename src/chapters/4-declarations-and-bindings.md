...
この章では、Haskellの **宣言** の構文と簡略したセマンティクスを説明する。
...
Haskellは静的型セマンティクス[4](../bibliography.md),[6](../bibliography.md)を提供するために伝統的なHindley-Milner多相型システムを使用するが、その型システムは構造化された手法にオーバーロード関数を導入するために提供する **型クラス** (または **クラス** )で拡張されている。
...
もし変数`f`が対応する型シグネチャ宣言を提供しないで定義されるなら、その時それ自身の宣言グループ(セクション[4.5](#a関数とパターン束縛の静的なセマンティクス))の外側の各`f`の使用は対応する推論される型、または **主要な** 型を持つように扱われる。
...
関数の静的セマンティクスと`let`式または`where`句のパターン束縛はこのセクションで論じる。

### 依存の解析

一般的に静的セマンティクスは通常のHindley-Milner推論規則を適用することによって与えられる。
多相性を増加するために、これらの規則は **依存の解析** によって見分けられる束縛のグループに適用される。

もし次のいずれかであるなら、束縛<em>b1</em>は同じ宣言のリストの中の束縛<em>b2</em>に依存する。

1. <em>b1</em>は型シグネチャを持たないかつ<em>b2</em>によって束縛される自由な識別子を含む。または、
...
その一般化の手順は多相的な型 ∀ <em>a.  a  →  (a,a)</em>という`g`に帰属し、その後に`"..."`の部分の型付けは続行可能である。

定義をオーバーロードされた型付けをする時、グループに宣言された各変数の型へ文脈を形成するために、単一の宣言グループからオーバーロードしている制約の全ては一緒に集められる。
例えば、以下の定義において
...
もしプログラマーが明示的な宣言グループの中の一つ以上の変数に型シグネチャを与えるなら、これらのシグネチャのその文脈は型変数の改名することに至るまで同一でなければならない。

### コンテキストの削減エラー

セクション[4.1.4](#a型とクラスのセマンティクス)で言及されたように、型のコンテキストは型変数または一つ以上の型変数の適用のみ制約できる。
よって、一般化によって生成された型は全てのコンテキストの制約が「Head Normal Form」に削減されなければいけない形式に表現されなければならない。
例として、次の定義を考えてもらいたい。
...
その等しさはリスト型で見られるのだが、そのコンテキストは一般化の前にリスト上の`Eq`にインスタンス宣言を使用するため簡易化されなければならない。
もしそのようなインスタンスがスコープ内に無いなら、静的エラーが起きる。

ここには<em>m</em>が一般化された型変数のひとつである形式<em>C (m t)</em>の制約のための要求を示す例がある。
すなわち、その形式においてのクラス<em>C</em>は型変数または型コンストラクタではない型式に適用する。
次のものを考えてもらいたい。
...
`f`の型は従って`(Monad m, Eq (m a)) => a -> m a -> Bool`であるべきで、そのコンテキストはそれ以上簡易化されることはできない。

データ型`deriving`句(セクション[4.3.3](#a導出されたインスタンス))から派生したインスタンス宣言は、あらゆるインスタンス宣言のように、**簡単なコンテキスト**を持たなければならない。
...
派生したShowインスタンスはコンテキスト`Show (a b)`を生成し、そしてそれは削減されることはできず、かつシンプルではないゆえに静的エラーが結果として生じる。

### 単相性

時々、その定義の型の中に使用される全ての型変数を超えて一般化することは可能ではない。
例えば、次の宣言を考える。
...
そのような単相性の効果は`g`の全ての適用の初めの引数が単一の型でなければならない。
例えば、次の式は`”...”`に有効であろう。
...
Haskellによって用意される明示的な型シグネチャは単相的な型変数を含む型を表現することには十分な性能ではないため価値がない。
例えば、次のものは書くことができない。
...
なぜなら、あれは`g`が`a`と`b`の両方に多相的であることを要求されるからだ(セクション[4.4.1](#a型シグネチャ))。
この問題の中で、もし最初の引数が型変数に関係する型に制限されるなら、`g`は型シグネチャのみ与えられる。
例えば、
