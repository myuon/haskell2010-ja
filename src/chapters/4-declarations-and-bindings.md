...
セクション[4.1.4](#a型とクラスのセマンティクス)で言及されたように、型の文脈は型変数または一つ以上の型変数の適用のみ制約できる。
よって、一般化によって生成された型は全ての文脈の制約が「Head Normal Form」に削減されなければいけない形式に表現されなければならない。
例として、次の定義を考えてもらいたい。
...
その等しさはリスト型で見られるのだが、その文脈は一般化の前にリスト上の`Eq`にインスタンス宣言を使用するため簡易化されなければならない。
...
派生したShowインスタンスは文脈`Show (a b)`を生成し、そしてそれは削減されることはできず、かつシンプルではないゆえに静的エラーが結果として生じる。
