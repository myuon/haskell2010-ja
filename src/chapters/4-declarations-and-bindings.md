...
等号はリストに対して使われているが、文脈は簡単な形にしなければならず、一般化の前にリストに対する`Eq`のインスタンス宣言を用いて行う。
...
Haskellは上記の標準Hindley-Milner制限の範囲を超えて、一般化の手順に一定の余分な制限を配置し、このケースに限り多相性をさらに削減する。
...
- **ルール1**<br>
    与えられた宣言グループが制限されないことを述べる。
    
    - **(a):**<br>
        グループ内の各変数は関数束縛または単純なパターン束縛によって束縛される(セクション[4.4.3.2](パターン束縛))。かつ、
    - **(b):**<br>
        明示的な型シグネチャは単純なパターン束縛によって束縛されているグループ内の各変数に与えられる。
    
    多相性上の普通のHindley-Milner制限は環境に自由に発生しない型変数のみが一般化されることである。
    さらに、制限された宣言グループの一般化の手順では**そのグループの制限された宣言グループの制約された型変数は一般化できない**。
    (もし同じ型クラスに属さないといけないなら、型変数が制約されていることを思い出すこと。セクション[4.5.2](#a一般化)を参照。)

- **ルール2**<br>
    全部のモジュールへの型推論が完了した時に残るあらゆる単相的な型変数は曖昧だと考えられ、そしてにデフォルトのルールを使用した各々の型に解決される(セクション[4.3.4](#a曖昧な型とオーバーロードされた数値演算子の既定値))。
</div>
...
- **ルール1は思いがけなく繰り返されることから計算を防止する。**
    ...
    もしプログラマが実際に繰り返される計算を願うなら、次のように明示的な型シグネチャが追加できる。
...
ルール2はモジュール内に束縛された全変数の正確な型がそのモジュールのみで決定されなければならないことを述べ、それをインポートするようなモジュールはない。
...
**結果**
単相性ルールはプログラマのために結果の数を持つ。
関数構文を伴い定義されたものはどれも関数が来るものとして予期されるように通常一般化する。
例として、
...
そのユーザーは完全にオーバーロードするものを保持するため型シグネチャを伴うこれらを添えることに注意しなければいけない。
